...
public class HuffTree {
    ...
    private void decode(){
        //get the encoded binary string from "huff_coded"
        Parser p = new Parser("huff_coded");
        ArrayList<String> encoded = p.extractString();

        /*For each line of encoded text iterate through it in such a way that reads each binary letter code individually.
        * The way this is done is with a while loop and a string builder.
        * A single letter is produced when each bit between the first bit and '.' is appended to the string builder.
        * Add the result of this process to a list of encoded characters then clear the string builder.
        * After each encoded character has been gathered into a single array, fun that list through a foreach loop that
        * uses the binary to char hashmap to decoded the encoded characters and append each character to a string. That
        * string is the original line from the "source_text"*/
        ...
        for(String line : encoded) {
            ...
            if (line.length() != 0) {
                ...
                //add the last encoded character
                codeArray.add(sb.toString());
            }
